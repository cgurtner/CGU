<?php

$PAGES = array();

$PAGES[] = array(
    "meta" => array(
        "title" => "Home",
        "description" => "Ich, Cyrill Gurtner, bin ein schweizer (Senior-) Softwareentwickler und eidg. dipl. Berufsbildner in Lehrbetrieben."
    ),
    "url" => "home",
    "path" => "content/page/home.php",
    "type" => "page",
    "display" => false,
    "disabled" => false
);

$PAGES[] = array(
    "meta" => array(
        "title" => "Werdegang",
        "description" => "Der Werdegang (CV) von mir, Cyrill Gurtner."
    ),
    "url" => "werdegang",
    "path" => "content/page/werdegang.php",
    "type" => "page",
    "display" => true,
    "disabled" => false
);

$PAGES[] = array(
    "meta" => array(
        "title" => "Kontakt",
        "description" => "Webapplikationen, komplexe Softwarelösungen, Datenbankmodelle und Dozententätigkeiten. Nehmen Sie mit mir Kontakt auf und besprechen Sie mit mir Ihre Ideen!"
    ),
    "url" => "kontakt",
    "path" => "content/page/kontakt.php",
    "type" => "page",
    "display" => true,
    "disabled" => false
);