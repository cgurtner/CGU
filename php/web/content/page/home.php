<main>
    <div class="container">
        <section>
            <div class="row">
                <div class="col pt-5 text-center">
                    <header>
                        <h1>Cyrill Gurtner</h1>
                        <h2 class="unset-background">Software Developer<span class="blink">_</span></h2>
                    </header>
                    <div class="social-links" itemscope itemtype="http://schema.org/Person">
                        <link itemprop="url" href="<?php echo CONFIG['baseURL']; ?>">
                        <a data-toggle="tooltip" data-placement="bottom" title="Mein GitLab"
                           itemprop="sameAs" href="<?php echo CONFIG['gitlab']; ?>" target="_blank">
                            <i class="fab fa-gitlab"></i>
                        </a>
                        <a data-toggle="tooltip" data-placement="bottom" title="Mein Instagram"
                           itemprop="sameAs" href="<?php echo CONFIG['instagram']; ?>" target="_blank">
                            <i class="fab fa-instagram"></i>
                        </a>
                    </div>
                </div>
            </div>
        </section>
    </div>
</main>